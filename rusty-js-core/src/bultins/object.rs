...
fn hash_<T>(v:T) -> u64 where T:Hash{
...
impl JObject{
    ...
    pub fn set_property_static(&mut self, key_id:u32, value:JValue, stack:*mut JValue){
        unsafe{self.inner.to_mut().set_property_static(key_id, value, stack)};
    }
    ...
    pub fn bind_getter(&self, id:u32, getter:JObject){
        if let Some((flag, cell)) = self.inner.to_mut().values.get_mut(&PropKey(id)){
            ...
            if flag.is_getter() && flag.is_setter(){
                *cell = PropCell{getsetter:(getter, unsafe{cell.getsetter.1})};
            } else if flag.is_setter(){
                *cell = PropCell{getsetter:(getter, unsafe{cell.setter})};
            } else{
                *cell = PropCell{getter:getter};
            };
            *flag = *flag|PropFlag::GETTER;
        } else{
            self.inner.to_mut().values.insert(PropKey(id), (PropFlag::GETTER|PropFlag::THREE, PropCell{getter:getter}));
        }
    }
    ...
    pub fn bind_setter(&self, id:u32, setter:JObject){
        if let Some((flag, cell)) = self.inner.to_mut().values.get_mut(&PropKey(id)){
            ...
            if flag.is_getter() && flag.is_setter(){
                *cell = PropCell{getsetter:(unsafe{cell.getsetter.0}, setter)};
            } else if flag.is_getter(){
                *cell = PropCell{getsetter:(unsafe{cell.getter}, setter)};
            } else{
                *cell = PropCell{setter:setter};
            };
            *flag = *flag|PropFlag::SETTER;
        } else{
            self.inner.to_mut().values.insert(PropKey(id), (PropFlag::SETTER|PropFlag::THREE, PropCell{setter:setter}));
        }
    }

    pub fn Call(&self, runtime:&Runtime, this:JValue, stack:*mut JValue, argc:u32) -> (JValue, bool) {
...
impl JObjectInner{
    ...
    pub fn set_static(&'static self, key:u32, value:JValue, stack:*mut JValue) -> (JValue, bool){
    ...
    pub unsafe fn get_property_static(&'static self, key:u32, stack:*mut JValue) -> (JValue, bool){
    ...
    pub fn insert_property(&mut self, key:&str, value:JValue, flag:PropFlag){
        ...
        self.values.insert(PropKey(r), (flag, PropCell{value:value}));
    }

    pub fn set_property(&'static mut self, key:&str, value:JValue, stack:*mut JValue) -> (JValue, bool){
        if key == "__proto__"{
            if value.is_object(){
    ...
    pub unsafe fn set_property_static(&'static mut self, key:u32, value:JValue, stack:*mut JValue) -> (JValue, bool){
        ...
        if let Some((f, v)) = self.values.get_mut(&PropKey(key)){
            ...
            self.values.insert(PropKey(key), (PropFlag::THREE, PropCell{value:value}));
        }
    ...
    pub fn get_own_property_descriptors(&self) -> PropMap{
        ...
        for (hashing, (flag, value)) in &self.values{
            ...
            if !flag.is_getter() && !flag.is_setter(){
                ...
                h.insert(*hashing, (PropFlag::THREE, PropCell{value:JValue::Object(o)}));
    ...
    pub fn Call(&self, runtime:&Runtime, this:JValue, stack:*mut JValue, argc:usize) -> (JValue, bool){
