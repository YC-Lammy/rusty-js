...
impl JSFunctionInstance{
    pub fn Call(&self, runtime:&Runtime, this:JValue, stack:*mut JValue, argc:usize) -> (JValue, bool){
        ...
        let this = if let Some(t) = self.this{
            t
        } else{
            this
        };
...
impl CaptureStackInner{
    ...
    fn as_data(&'static mut self) -> &'static mut [JValue]{
        let ptr = unsafe{(self as *mut Self).add(1) as *mut JValue};
        let len = (self.alloc - std::mem::size_of::<CaptureStackInner>())/ std::mem::size_of::<JValue>();
    ...
    fn drop(&mut self) {
        match self{
            Self::Allocated(a) => {
                a.to_mut().rc -= 1;
                if a.rc == 0{
                    unsafe{std::alloc::dealloc(a.to_mut() as *mut CaptureStackInner as *mut u8, Layout::array::<u8>(a.alloc).unwrap())};
                }
            },
            Self::NeedAlloc(_) => {},
            Self::None => {}
...
impl JSFunction{
    ...
    fn capture_stack_size(&self) -> Option<u16>{
        match self{
            ...
            Self::Baseline { capture_stack_size, ..} => {
                *capture_stack_size
            },
            Self::Native(_) => None,
            Self::ByteCodes{capture_stack_size, ..} => *capture_stack_size,
        }
    }

    pub fn create_instance_with_capture(self:Arc<Self>, this:Option<JValue>, capture_stack:*mut JValue) -> JSFunctionInstance{
    ...
    pub fn Call(&self, runtime:&Runtime, this:JValue, stack:*mut JValue, argc:usize, capture_stack:&mut [JValue]) -> (JValue, bool){
        match self.to_mut(){
            ...
            Self::ByteCodes { 
                ...
                *call_count += 1;
                ...
                let stack_ = unsafe{std::slice::from_raw_parts_mut(stack.add(argc), size as usize)};
                let args = unsafe{std::slice::from_raw_parts(stack, argc)};

                let mut intpr = crate::interpreter::Interpreter::function(runtime, stack_, capture_stack);
            ...
            Self::Baseline { 
                ...
                *call_count += 1;
                func.call(runtime, this, argc as u32, stack, capture_stack.as_mut_ptr())
            },
