...
use crate::{utils::nohasher, types::JValue, JObject, Runtime, error::Error};

use super::{function::{JSFunction, JSFunctionInstance}, prop::PropFlag, object::JObjectValue};
...
impl JSClass {
    ...
    pub fn create_object_with_capture(self:Arc<Self>, capture_stack:*mut JValue) -> JObject{
        ...
        for key in &self.props{
            prototype.insert_property_static(*key, JValue::UNDEFINED, PropFlag::CONFIGURABLE | PropFlag::WRITABLE);
        }
        
        for (k, f) in &self.methods{
            let f = f.clone().create_instance_with_capture(None, capture_stack);
            prototype.insert_property_static(*k, JObject::with_function(f).into(), PropFlag::CONFIGURABLE | PropFlag::WRITABLE);
        };
        ...
        for key in &self.static_props{
            obj.insert_property_static(*key, JValue::UNDEFINED, PropFlag::CONFIGURABLE | PropFlag::WRITABLE);
        };
        ...
        for (k, f) in &self.static_methods{
            let f = f.clone().create_instance_with_capture(None, capture_stack);
            obj.insert_property_static(*k, JObject::with_function(f).into(), PropFlag::CONFIGURABLE | PropFlag::WRITABLE);
        };

        obj.insert_property("prototype", prototype.into(), PropFlag::NONE);
        obj.insert_property("name", JValue::String(self.name.as_str().into()), PropFlag::CONFIGURABLE);
    ...
    pub fn create_object_without_capture(self:Arc<Self>) -> JObject{
        ...
        prototype.insert_property("constructor", obj.into(), PropFlag::CONFIGURABLE | PropFlag::WRITABLE);
