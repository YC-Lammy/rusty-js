...
impl FunctionBuilderContext {
    ...
    pub fn need_done(&self) -> Vec<OpCode> {
        let mut v = self.inner.to_mut().need_done.take();
    ...
    pub fn close_context(&mut self) {
        if let Some(p) = &self.inner.parent {
            if self.inner.need_done.len() != 0 {
                panic!(
                    "Bytecode builder context has not yet done every job. {:?}",
                    &self.inner.need_done
                )
            }
            self.inner = p.clone();
        } else {
...
impl FunctionBuilderContextInner {
            ...
            match v {
                ...
                Variable::Const(off) => {
                    if !self.is_global_context {
                        ...
                        let _id = runtime.to_mut().regester_dynamic_var_name(name.0.as_ref());

                        return OpCode::ReadDynamicVarDeclared {
                            result: write_to,
                            offset: *off,
                        };
                    }
            ...
            if let Some(p) = &parent {
                ...
                if let Some(v) = p.variables.get(&name) {
                    match v {
                        ...
                        Variable::Const(off) | Variable::Let(off) | Variable::Var(off) => {
                            if need_capture && !is_global {
                                let offset = self.get_next_capture_offset();

                                p.to_mut().need_done.push(OpCode::ReadFromStack { 
                                    result: Register(2), 
                                    stack_offset: *off 
                                });

                                p.to_mut().need_done.push(OpCode::WriteCapturedVar { 
                                    from: Register(2), 
                                    offset: offset
                                });
    ...
    fn set(&mut self, name: Id, from: Register) -> OpCode {
        if let Some(v) = self.variables.get(&name) {
            match v {
                ...
                Variable::Var(off) | Variable::Let(off) => {
                    if self.is_global_context {
                        let runtime = Runtime::current();
            ...
            if let Some(p) = &parent {
                ...
                if let Some(v) = p.variables.get(&name) {
                    match v {
                        ...
                        Variable::Let(off) | Variable::Var(off) => {
                            if need_capture && !is_global {
                                let offset = self.get_next_capture_offset();
                                ...
                                return OpCode::WriteDynamicVarDeclared {
                                    from: from,
                                    offset: *off,
                                };
                            } else {
    ...
    pub fn declare(&mut self, name: Id, from: Register, kind: DeclareKind) -> OpCode {
        ...
        if self.is_global_context {
            let runtime = Runtime::current();
            let _id = runtime.regester_dynamic_var_name(name.0.as_ref());
