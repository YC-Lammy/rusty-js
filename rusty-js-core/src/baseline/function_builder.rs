...
use cranelift::prelude::{InstBuilder, MemFlags, isa, AbiParam, StackSlotData, StackSlotKind};
...
use crate::bytecodes::{OpCode, LoopHint, TempAllocValue};
...
static NOT_USE_FUNC_CTXS:RwLock<Vec<&'static mut FunctionBuilderContext>> = RwLock::new(Vec::new());
...
impl JSFunctionBuilder{
    fn new() -> Self{
        ...
        let mut this = Self{
            ...
            args_len:Variable::with_u32(5),

            stack_pointer:Variable::with_u32(6),
            capture_stack_pointer:Variable::with_u32(7),

            registers:[
                JSVariable(Variable::with_u32(8), Variable::with_u32(9)),
                JSVariable(Variable::with_u32(10), Variable::with_u32(11)),
                JSVariable(Variable::with_u32(12), Variable::with_u32(13)),
            ],
            error:Variable::with_u32(14),
            ...
            iter_ended:Variable::with_u32(15),
        ...
        this.define_var(13, POINTER_TYPE);

        this.define_var(14, types::B8);
        this.define_var(15, types::B8);

        // fn(JValueUnion, JValue) -> (JValue, bool)
        this.basic_op_sigref = this.builder.import_signature(Signature { 
            params: vec![AbiParam::new(POINTER_TYPE), AbiParam::new(POINTER_TYPE), AbiParam::new(POINTER_TYPE)], 
            returns: vec![AbiParam::new(POINTER_TYPE), AbiParam::new(POINTER_TYPE), AbiParam::new(types::B8)], 
            call_conv: ISA.default_call_conv()
        });

        this.into_iter_sig = this.builder.import_signature(Signature { 
            params: vec![AbiParam::new(POINTER_TYPE), AbiParam::new(POINTER_TYPE), AbiParam::new(types::I8)], 
        ...
        this.iter_next_sig = this.builder.import_signature(Signature { 
            params: vec![
                ...
                AbiParam::new(POINTER_TYPE), AbiParam::new(POINTER_TYPE), //JValue
        ...
        this.builder.def_var(this.args_pointer, params[3]);
        this.builder.def_var(this.args_len, params[4]);
    ...
    fn store_error(&mut self, error:ir::Value){
        self.builder.def_var(self.error, error);
    }

    fn call_vtable(&mut self, vtable:ir::Value, offset:i32, sigref:SigRef, args:&[ir::Value]) -> CallResult{
        let addr = self.builder.ins().load(POINTER_TYPE, MemFlags::new(), vtable, offset);
        let inst = self.builder.ins().call_indirect(sigref, addr, args);
        let re = self.builder.inst_results(inst);
        CallResult { value: re[0], vtable: re[1], error: re[2] }
    }
    ...
    fn read_stack(&mut self, offset:u16) -> (ir::Value, ir::Value){
        ...
        let v = self.builder.ins().load(POINTER_TYPE, MemFlags::new(), stack, offset as i32);
        let t = self.builder.ins().load(POINTER_TYPE, MemFlags::new(), stack, (offset + std::mem::size_of::<usize>()) as i32);
        (v, t)
    }
    ...
    fn translate_byte_code(&mut self, code:&OpCode){
        match code{
            ...
            OpCode::Add { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_add(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::Sub { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_sub(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::Mul { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_mul(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::Div { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_div(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::Rem { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_rem(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::EqEq { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_eqeq(), self.basic_op_sigref, &[l.0, r.0, r.1]);

                self.store_error(re.error);
                self.store_idx(result.0, re.value, re.vtable);
            },
            OpCode::NotEq { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);
            ...
            OpCode::EqEqEq { result, left, right } => {
                ...
                let true_vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::TRUE as *const _ as usize as i64);
                let false_vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::FALSE as *const _ as usize as i64);

                let a = self.builder.ins().icmp(ir::condcodes::IntCC::Equal, l.0, r.0);
                let b = self.builder.ins().icmp(ir::condcodes::IntCC::Equal, l.1, r.1);
            ...
            OpCode::NotEqEq { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);
            ...
            OpCode::Exp { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_exp(), self.basic_op_sigref, &[l.0, r.0, r.1]);

                self.store_error(re.error);
                self.store_idx(result.0, re.value, re.vtable);
            },
            OpCode::InstanceOf { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);

                let re = self.call_vtable(l.1, JTypeVtable::offset_instanceOf(), self.basic_op_sigref, &[l.0, r.0, r.1]);

                self.store_error(re.error);
                self.store_idx(result.0, re.value, re.vtable);
            },
            OpCode::In { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);
                
                // the right hand side is the caller
                let re = self.call_vtable(r.1, JTypeVtable::offset_In(), self.basic_op_sigref, &[l.0, r.0, r.1]);

                self.store_error(re.error);
                self.store_idx(result.0, re.value, re.vtable);
            },
            OpCode::Gt { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);

                let re = self.call_vtable(l.1, JTypeVtable::offset_gt(), self.basic_op_sigref, &[l.0, r.0, r.1]);

                self.store_error(re.error);
                self.store_idx(result.0, re.value, re.vtable);
            },
            OpCode::GtEq { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);

                let re = self.call_vtable(l.1, JTypeVtable::offset_gteq(), self.basic_op_sigref, &[l.0, r.0, r.1]);

                self.store_error(re.error);
                self.store_idx(result.0, re.value, re.vtable);
            },
            OpCode::Lt { result, left, right } => {
                let l = self.use_idx(left.0);
                let r = self.use_idx(right.0);

                let re = self.call_vtable(l.1, JTypeVtable::offset_lt(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::LtEq { result, left, right } => {
                ...
                let re = self.call_vtable(l.1, JTypeVtable::offset_lteq(), self.basic_op_sigref, &[l.0, r.0, r.1]);
            ...
            OpCode::And { result, left, right } => {
                ...
                let a = self.builder.ins().icmp_imm(ir::condcodes::IntCC::NotEqual, l.0, 0);
                let b = self.builder.ins().icmp_imm(ir::condcodes::IntCC::NotEqual, r.0, 0);
                let c = self.builder.ins().band(a, b);
                let vtable = self.builder.ins().select(c, true_vtable, false_vtable);
            ...
            OpCode::BitAnd { result, left, right } => {
                todo!("bitand")
            },
            OpCode::BitOr { result, left, right } => {
                todo!("bitor")
            },
            OpCode::BitXor { result, left, right } => {
                todo!("bitXor")
            }
            ...
            OpCode::Select { a, b, result } => {
                ...
                let c = self.builder.ins().icmp_imm(ir::condcodes::IntCC::Equal, a.1, JValue::UNDEFINED.type_pointer as *const _ as usize as i64);
            ...
            OpCode::Capture { stack_offset, capture_stack_offset } => {
                // read from offset and write to dynamic var

                let ptr = self.builder.use_var(self.stack_pointer);
                let offset = (*stack_offset as usize) * std::mem::size_of::<JValue>();

                let capture_offset = (*capture_stack_offset as usize) *std::mem::size_of::<JValue>();

                let value = self.builder.ins().load(POINTER_TYPE, MemFlags::new(), ptr, offset as i32);
                let vtable = self.builder.ins().load(POINTER_TYPE, MemFlags::new(), ptr, (offset + std::mem::size_of::<usize>()) as i32);

                todo!()
            },
            OpCode::ReadDynamicVar { result, id } => {
                let callee = self.builder.ins().iconst(POINTER_TYPE, self.dynamic_get as *const u8 as usize as i64);
                let runtime = self.builder.use_var(self.runtime);
                let key = self.builder.ins().iconst(types::I32, *id as i64);

                let inst = self.builder.ins().call_indirect(self.dynamic_get_sigref, callee, &[runtime, key]);
            ...
            OpCode::WriteDynamicVar { from, id } => {
                ...
                let callee = self.builder.ins().iconst(POINTER_TYPE, self.dynamic_set as *const u8 as usize as i64);
                let runtime = self.builder.use_var(self.runtime);
                let key = self.builder.ins().iconst(types::I32, *id as i64);

                self.builder.ins().call_indirect(self.dynamic_set_sigref, callee, &[runtime, key, v.0, v.1]);
            },
            ...
            OpCode::StoreTempAlloc { offset, flag, value } => {
                let slot = *self.temp_allocs.last().expect("store temp alloc before allocating");
                let v = self.use_idx(value.0);
                ...
                self.builder.ins().stack_store(v.1, slot, off + 1 + std::mem::size_of::<usize>() as i32);
            },
            OpCode::ReadTempAlloc { offset, result } => {
                let off = *offset as i32 * std::mem::size_of::<TempAllocValue>() as i32;
                let slot = *self.temp_allocs.last().expect("read temp alloc before allocating");

                let value = self.builder.ins().stack_load(POINTER_TYPE, slot, off+1);
                let vtable = self.builder.ins().stack_load(POINTER_TYPE, slot, off + 1 + std::mem::size_of::<usize>() as i32);
                self.store_idx(result.0, value, vtable);
            },
            OpCode::TempDealloc { size } => {
                self.temp_allocs.pop().expect("temp dealloc without allocating");
            },

            OpCode::WriteToStack { from, stack_offset } => {
                let ptr = self.builder.use_var(self.stack_pointer);
                let offset = *stack_offset as usize * std::mem::size_of::<usize>() *2;
                let v = self.use_idx(from.0);
                self.builder.ins().store(MemFlags::new(), v.0, ptr, offset as i32);
                self.builder.ins().store(MemFlags::new(), v.1, ptr, (offset + std::mem::size_of::<usize>()) as i32);
            },
            ...
            OpCode::IntoIter { target, hint } => {
                ...
                let callee = self.builder.ins().iconst(POINTER_TYPE, self.into_iter as *const u8 as usize as i64);
                let hint = self.builder.ins().iconst(types::I8, *hint as u8 as i64);

                let inst = self.builder.ins().call_indirect(self.into_iter_sig, callee, &[v.0, v.1, hint]);
            ...
            OpCode::IterNext { result, hint, stack_offset } => {

                let callee = self.builder.ins().iconst(POINTER_TYPE, self.iter_next as *const u8 as usize as i64);

                let iter = self.builder.ins().stack_load(POINTER_TYPE, *self.iter.last().unwrap(), 0);
                let this = (self.builder.use_var(self.this.0), self.builder.use_var(self.this.1));

                let stack = self.builder.use_var(self.stack_pointer);
                let stack = self.builder.ins().iadd_imm(stack, *stack_offset as i64);
            ...
            OpCode::IterCollect { result, stack_offset } => {
                let callee = self.builder.ins().iconst(POINTER_TYPE, self.iter_collect as *const u8 as usize as i64);
            ...
            OpCode::IterDrop => {
                let callee = self.builder.ins().iconst(POINTER_TYPE, self.iter_drop as *const u8 as usize as i64);
                let slot = self.iter.pop().unwrap();
                let iter = self.builder.ins().stack_load(POINTER_TYPE, slot, 0);

                self.builder.ins().call_indirect(self.iter_drop_sig, callee, &[iter]);
            },
            ...
            OpCode::LoadFalse { result } => {
                let vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::FALSE as *const _ as usize as i64);
                let value = self.builder.ins().iconst(POINTER_TYPE, 0);
                self.store_idx(result.0, value, vtable);
            },
            OpCode::LoadTrue { result } => {
                let vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::TRUE as *const _ as usize as i64);
                let value = self.builder.ins().iconst(POINTER_TYPE, 0);
                self.store_idx(result.0, value, vtable);
            },
            OpCode::LoadNull { result } => {
                let vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::NULL as *const _ as usize as i64);
                let value = self.builder.ins().iconst(POINTER_TYPE, 0);
                self.store_idx(result.0, value, vtable);
            },
            OpCode::LoadUndefined { result } => {
                let vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::UNDEFINED as *const _ as usize as i64);
                let value = self.builder.ins().iconst(POINTER_TYPE, 0);
                self.store_idx(result.0, value, vtable);
            },
            ...
            OpCode::LoadStaticBigInt32 { result, value } => {
                let vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::BIGINT as *const _ as usize as i64);
            ...
            OpCode::LoadStaticFloat32 { result, value } => {
                let vtable = self.builder.ins().iconst(POINTER_TYPE, &JTypeVtable::BIGINT as *const _ as usize as i64);
                let value = self.builder.ins().iconst(POINTER_TYPE, unsafe{std::mem::transmute::<_, i64>(*value as f64)});
                self.store_idx(result.0, value, vtable);
            },
