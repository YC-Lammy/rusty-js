...
pub struct FunctionBuilderContext{
    inner:Arc<FunctionBuilderContextInner>
}
...
impl FunctionBuilderContextInner{
    ...
    pub fn get(&mut self, name:&Id, write_to:Register) -> OpCode{
        if let Some(v) = self.variables.get(&name){
            match v{
                Variable::Captured(off) => {
                    return OpCode::ReadCapturedVar { result: write_to, offset: *off }
                },
                Variable::Const(off) => {
                    return OpCode::ReadFromStack { result: write_to, stack_offset: *off}
                },
                Variable::Let(off) => {
                    return OpCode::ReadFromStack{ result: write_to, stack_offset: *off }
                },
                    ...
                    match v{
                        ...
                        Variable::Const(off) => {
                            if need_capture && !is_global{
                                ...
                                p.to_mut().variables.insert(name.clone(), Variable::Captured(offset));
        ...
        return OpCode::ReadDynamicVar { result: write_to, id: id }
    }
    ...
    fn set(&mut self, name:Id, from:Register) -> OpCode{
        if let Some(v) = self.variables.get(&name){
            match v{
                Variable::Captured(off) => {
                    return OpCode::WriteCapturedVar { from: from, offset: *off }
                },
                Variable::Const(off) => {
                ...
                Variable::Let(off) => {
                    ...
                    return OpCode::WriteToStack { from: from, stack_offset: *off }
                },
                Variable::Var(off) => {
                    if self.is_global_context{
                        let runtime = Runtime::current();
                    ...
                    match v{
                        ...
                        Variable::Var(v) => {
                            if need_capture && !is_global{
                                ...
                                let runtime = Runtime::current();
                                let id = runtime.to_mut().regester_dynamic_var_name(&format!("{}#{}", name.0.as_ref(), name.1.as_u32()));
    ...
    pub fn declare(&mut self, name:Id, from:Register, kind:DeclareKind) -> OpCode{
        ...
        if !self.is_global_context{
            OpCode::WriteToStack { from: from, stack_offset: off }
        } else{
            let runtime = Runtime::current();
            let id = runtime.regester_dynamic_var_name(name.0.as_ref());

            OpCode::DeclareDynamicVar { from: from, kind: kind, id: id }
        }
