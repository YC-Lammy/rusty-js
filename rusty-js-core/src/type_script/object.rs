...
impl ObjectInfo{
    ...
    pub fn creat_object(&self) -> TSObject{
        unsafe{
            ...
            return TSObject { inner: first_fragment.as_ref().unwrap() }
        }
    }
...
impl TSObject{
    fn get_header(&self) -> &mut TSObjectHeader{
        ...
        unsafe{(ptr.sub(offset as usize) as *mut TSObjectHeader).as_mut().unwrap()}
    }

    pub fn has_parent(&self) -> bool{
        let l = self.get_header().total_length as i32;
        return (l - self.inner.offset as i32 - self.inner.length as i32 - std::mem::size_of::<TSObjectFragment>() as i32) > 0
    }
