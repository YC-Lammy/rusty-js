...
impl<T> AsyncExecutor<T>{
    pub fn new() -> Self{
        Self { jobs: Default::default(), next_id: 0 }
    }
}
...
impl<T: 'static> AsyncExecutor<T>{
    ...
    pub fn run<F>(&mut self, func:F, start:bool) -> AsyncId where F:Fn() -> Result<T, T> + 'static{
        let id = self.next_id;
        self.next_id += 1;
    ...
    pub fn poll(&mut self, id:AsyncId, input:T) -> bool{
        if let Some(state)  = self.jobs.get_mut(&id.0){
            match state{
                ...
                AsyncState::Resolved(_) => {
                    return true
                },
                AsyncState::Rejected(_) => {
                    return false
                }
            };
        } else{
                ...
                AsyncState::Pending(p) => {
                    ...
                    match re{
                        ...
                        CoroutineResult::Yield(v) => {
                            return AsyncResult::Yield(v)
                        }
                    }
                },
                AsyncState::Resolved(r) => {
                    return AsyncResult::Return(r.clone())
                },
                AsyncState::Rejected(r) => {
                    return AsyncResult::Err(r.clone())
                }
            };
        } else{
