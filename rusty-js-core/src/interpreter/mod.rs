...
use crate::bytecodes::{OpCode, Register, Block, TempAllocValue};
...
impl<'a> Interpreter<'a>{
    ...
    pub fn function(runtime:&'a Runtime, stack:&'a mut [JValue], capture_stack:&'a mut [JValue]) -> Self{
        Self { 
            runtime, 
            r: Registers([JValue::UNDEFINED; 3]), 
            stack: stack, 
    ...
    pub fn run(&mut self, mut this:JValue, args:&[JValue], codes:&[OpCode]) -> Result<JValue, JValue>{
    ...
    #[inline]
    fn run_code(&mut self, this:&mut JValue, args:&[JValue], code:OpCode, index:&mut usize) -> Res{
        match code{
            OpCode::NoOp => {},
            OpCode::Debugger => {},
            ...
            OpCode::EqEqEq { result, left, right } => {
                self.r[result] = unsafe{std::mem::transmute::<_, [usize;2]>(self.r[left]) == std::mem::transmute::<_, [usize;2]>(self.r[right])}.into()
            },
            ...
            OpCode::Await { result, future } => {
                let (v, error) = self.r[future].wait();
                if error{
                    return Res::Err(v)
                }
                self.r[result] = v;
            ...
            OpCode::Nullish { result, left, right } => {
                let l = self.r[left];
            ...
            OpCode::Throw { value } => {
                return Res::Err(self.r[value])
            },
            OpCode::Return { value } => {
                return Res::Return(self.r[value])
            }
            ...
            OpCode::IterNext { result, hint:_ , stack_offset} => {
            ...
            OpCode::CreateArg { stack_offset, len:_ } => {
                self.args_offset = stack_offset as usize;
                self.args_len = 0;
            },
            OpCode::PushArg { value } => {
                self.stack[self.args_offset + self.args_len] = self.r[value];
                self.args_len += 1;
            },
            OpCode::PushArgSpread { value } => {
                let mut v = vec![];
                let iter = unsafe{FastIterator::new(self.r[value], crate::bytecodes::LoopHint::For)};
                loop{
                    let stack = &mut self.stack[self.args_offset + self.args_len..];
                    let (done, error, value) = iter.next(*this, stack.as_mut_ptr());
                    if error{
                        return Res::Err(value)
                    }
                ...
                let s = &mut self.stack[(self.args_offset + self.args_len)..];
                unsafe{std::ptr::copy(v.as_ptr(), s.as_mut_ptr(), v.len())};
                self.args_len += v.len();
            },
            ...
            OpCode::Call { result, this, callee, stack_offset:_ } => {
                let stack = &mut self.stack[self.args_offset..];
                let (v, error) = unsafe{
                    self.r[callee].call_raw(self.runtime, self.r[this], stack.as_mut_ptr(), self.args_len as u32)
                };
                if error{
                    return Res::Err(v)
                }
            ...
            OpCode::CallOptChain { result, this, callee, stack_offset:_ } => {
            ...
            OpCode::New { result, callee, stack_offset:_ } => {

                let stack = &mut self.stack[self.args_offset..];
                let (v, error) = unsafe{
                    self.r[callee].new_raw(self.runtime, stack.as_mut_ptr(), self.args_len as u32)
                };
                if error{
                    return Res::Err(v)
                }
            ...
            OpCode::ReadField { obj, field, result, stack_offset } => {
                let obj = self.r[obj];
            ...
            OpCode::ReadFieldOptChain { obj, field, result, stack_offset } => {
                let obj = self.r[obj];
            ...
            OpCode::ReadFieldStatic { obj_result, field_id, stack_offset } => {
            ...
            OpCode::ReadFieldStaticOptChain { obj_result, field_id, stack_offset } => {
                let obj = self.r[obj_result.target()];
            ...
            OpCode::WriteField { obj, field, value, stack_offset } => {
                let obj = self.r[obj];
                let field = self.r[field].to_string();
                let id = self.runtime.register_field_name(&field);
                let stack = &mut self.stack[stack_offset as usize..];
            ...
            OpCode::WriteFieldOptChain { obj, field, from, stack_offset } => {
                let obj = self.r[obj];
                if obj.is_null() || obj.is_undefined(){
            ...
            OpCode::WriteFieldStatic { obj_value, field_id, stack_offset } => {
            ...
            OpCode::BindGetter { obj, field_id, getter } => {
                let obj = self.r[obj];
                if obj.is_object(){
                    unsafe{obj.value.object.bind_getter(field_id, self.r[getter].value.object)}
                }
            },
            OpCode::BindSetter { obj, field_id, setter } => {
                let obj = self.r[obj];
                if obj.is_object(){
                    unsafe{obj.value.object.bind_setter(field_id, self.r[setter].value.object)}
                }
            },
            ...
            OpCode::CreateArrow { result, this, id } => {
                let f = if self.is_global{
                    self.runtime.get_function(id).unwrap().create_instance(Some(self.r[this]))
                } else{
                    let ptr = self.capture_stack.as_mut().unwrap().as_mut_ptr();
                    self.runtime.get_function(id).unwrap().create_instance_with_capture(Some(self.r[this]), ptr)
                };
            ...
            OpCode::CreateFunction { result, id } => {
                let f = if self.is_global{
                    ...
                    self.runtime.get_function(id).unwrap().create_instance_with_capture(None, self.capture_stack.as_mut().unwrap().as_mut_ptr())
                };
