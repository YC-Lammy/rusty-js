...
use crate::{bultins::{object::{JObject, JObjectValue}, prop::PropFlag}, types::JValue, error::Error, runtime::Runtime, bytecodes::LoopHint};
...
impl FastIterator{
    pub unsafe fn new(iter:JValue, hint:LoopHint) -> &'static mut FastIterator{
        Box::leak(Box::new(
        if hint == LoopHint::ForIn{

            if iter.is_object(){
                let keys = iter.value.object.keys();
                FastIterator::ForIn { keys: keys, count: 0 }
            } else{
                FastIterator::Empty
            }
            ...
            if iter.is_object(){
                if iter.value.object.is_array(){
                    FastIterator::Array { array: iter.value.object.inner.wrapped_value.array().unwrap().clone(), count: 0 }
                } else{
                    FastIterator::Object { obj: iter.value.object }
                }
            } else{
                FastIterator::Empty
            }
        ...
        match self{
            ...
            Self::Object { obj } => {
                let next = match obj.get_property("next", stack){
                    Some(v) => v,
                    None => return (false, true, JValue::Error(Error::InvalideIterator { 
                        msg: "" 
                    }))
                };
                ...
                match next.call(crate::bultins::function::JSFuncContext { stack: stack },this, &[]){
                    Ok(result) => {
            ...
            Self::ForIn { keys, count } => {
                ...
                if keys.len() > *count as usize + 1{
                    *count +=1;
                    ...
                    (false, false, JValue::String(runtime.get_field_name(key).into()))

                } else if keys.len() == *count as usize{

                    let key = keys[*count as usize];
                    let v = (true, false, JValue::String(runtime.get_field_name(key).into()));
                    *self = Self::Empty;
                    return v;
                } else{
                    (true, false, JValue::UNDEFINED)
